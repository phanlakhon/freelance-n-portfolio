...
export default async function ProductsPage({
    ...
    return (
        <main>
            ...
            <section className="section-container bg-neutral-50 py-14 md:py-20">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
                    ...
                    {result.status === "ready" && (
                        <p className="shrink-0 whitespace-nowrap text-sm text-neutral-500 md:text-right">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {products.map((product) => {
                            ...
                            return (
                                <article
                                    key={product.id}
                                    className="group flex h-full min-w-0 flex-col overflow-hidden bg-white border border-neutral-200 transition-all duration-300 hover:border-accent hover:shadow-lg"
                                >
                                    <Link
                                        href={productUrl}
                                    ...
                                    <div className="flex min-w-0 flex-1 flex-col p-6 md:p-7">
                                        <div className="mb-5 min-w-0">
                                            ...
                                            <p className="mt-3 inline-flex border border-neutral-200 bg-neutral-50 px-3 py-1 text-sm font-semibold text-neutral-900">
                                                {product.formatted_price}
                                            </p>
                                        </div>

                                        {description && (
                                            <p className="mb-6 line-clamp-4 min-w-0 break-words text-sm text-neutral-600 leading-relaxed">
                                                {description}
                                            </p>
                                        )}
                                        ...
                                        {product.tags.length > 0 && (
                                            <div className="flex flex-wrap gap-2 mb-7">
                                                {product.tags.slice(0, 4).map((tag) => (
                                                    <span
                                                        key={tag}
                                                        className="max-w-full truncate px-3 py-1 text-[10px] font-bold uppercase tracking-tight bg-neutral-100 text-neutral-600 border border-neutral-200"
                                                    >
                                                        {tag}
                                                    </span>
                                                ))}
